...
public class CameraController : MonoBehaviour {
    ...
    private string p1InputHori = "P1_L_Horizontal";
    private string p1InputVert = "P1_L_Vertical";
    ...
    private void Start()
    {
        ...
        {
            if(targets[i].GetComponent<PlayerIdentifier>())
            {
                if (targets[i].GetComponent<PlayerIdentifier>().player.Equals(PlayerIdentifier.PlayerNumber.Player1))
                {
                    target = targets[i];
                }
            }
        ...
        switch (mode) {
            ...
            case CameraMode.ThirdPerson: // Aiming Phase & Rolling Phase
                ...
                if (!target.GetComponent<PlayerController>().UsedTurn) {
                    ...
                    if(target != PlayerTurn.Instance.playerObjTurn)
                    {
                        target = PlayerTurn.Instance.playerObjTurn;
                    }
